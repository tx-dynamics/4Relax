...
var RNFS = require('react-native-fs');
    ...
    useEffect(() => {
        // alert(props?.userData?.subscriptionDetail?.subscriptionId)
  ...
  async function startDownload  (item,index)  {
    ...
    setmeditations(res)
      ...
      const tracktype = '/storage/emulated/0/Download/FourRelax/sounds'
    ...
    RNFetchBlob.fs.isDir(FolderPAth).then((isDir)=>{
      if(isDir){
        ...
        RNFetchBlob.fs.isDir(tracktype).then((isDir)=>{
          if(isDir){
            // alert('exist medii')
            let SongDir =RNFetchBlob.fs.dirs.DownloadDir+'/FourRelax/sounds'+ '/' + name
            RNFetchBlob.config({
              fileCache: true,
              appendExt: 'mp3',
              addAndroidDownloads: {
              ...
              //     notification: false,
              //     title: name+"_img",
              //     path : RNFetchBlob.fs.dirs.DownloadDir+'/FourRelax/sounds'+ '/' + name+"_img",
              //     // path: RNFetchBlob.fs.dirs.DownloadDir + `${name}`, // Android platform
              //     description: 'Image',
              //   },
              // })
                    ...
                    :
                      <>
                        ...
                        <>
                          {connection?
                            <View style={{flex:1,alignSelf:'center',alignItems:'center',justifyContent:'center'}}>
                              <Text style={styles.title} >Sound List is empty</Text>
                            </View>
                          :
                          <View style={{flex:1,alignSelf:'center',alignItems:'center',justifyContent:'center'}}>
